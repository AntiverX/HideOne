...
def main():
    ...
    if opt.test == '':
        ...
        opt.Hnet = "/tmp/pycharm_project_380/training/lab-PowerEdge-R740_2021-10-20-11_37_30 大小设置为64/checkPoints/netH_epoch_97,sumloss=0.000752,Hloss=0.000419.pth"
        opt.Rnet = "/tmp/pycharm_project_380/training/lab-PowerEdge-R740_2021-10-20-11_37_30 大小设置为64/checkPoints/netR_epoch_97,sumloss=0.000752,Rloss=0.000443.pth"
        testdir = opt.test
...
def train(train_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        betaerrR_secret = opt.beta * errR * BACKDOOR + opt.beta * errR_clean * CLEAN
        ...
        if epoch % 1 == 0 and i % opt.resultPicFrequency == 0:
            save_result_pic('train', 1, clean_recovered_imgv.data[1], cover_img[1], container_img.data[1], secret_img[1], rev_secret_img.data[1], epoch, i, opt.trainpics)
...
def validation(val_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(val_loader, 0):
        ...
        if i % 50 == 0:
            save_result_pic('validate', 1, clean_recovered_imgv.data[1], cover_img[1], container_img.data[1], secret_img[1], rev_secret_img.data[1], epoch, i, opt.validationpics)
...
def test(test_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        # 测试缩放
        # container_img = F.interpolate(F.interpolate(container_img, size=128), size=256)
        ...
        for i in range(this_batch_size):
            save_image(cover_img[i], '/tmp/cover{}.png'.format(i))
            save_image(container_img.data[i], '/tmp/container_img{}.png'.format(i))

        save_result_pic('validate', this_batch_size, cover_img, container_img.data, secret_img, rev_secret_img.data, epoch, i,
                        opt.testPics)
...
def save_result_pic(mode, this_batch_size, rev_secret_img, originalLabelv, ContainerImg, secretLabelv, RevSecImg, epoch, i, save_path):
    if not opt.debug:
        rev_secret_img = rev_secret_img.resize_(this_batch_size, 3, opt.imageSize, opt.imageSize)
        ...
        resultImg = torch.cat([rev_secret_img, showContainer, showReveal], 0)
        resultImgName = '%s/Result%sPics_epoch%03d_batch%04d.png' % (save_path, mode, epoch, i)
